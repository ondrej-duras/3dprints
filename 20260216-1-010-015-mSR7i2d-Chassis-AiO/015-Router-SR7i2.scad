/* [Router Nokia 7750-SR7-i2-fantasy mSR7-i2] */
// 20250707, Ondrej DURAS (dury) Nokia
// License : GNU GPL 2.0, PUBLIC document
// This file does not contains data from internal Nokia resources
// Whole design has been made on base of public eBay photo resources
// Between router in this file and real router are many differences
// Router in this file is intended to be a toy for kids only.
// ...so enjoy !
...
// Vyber pracovneho nahladu
setup="design_tray_fan"; // [design_tray_linecards, design_tray_fan, design_tray_psu, design_fan, design_psu, design_filters, design_cards, present_delabored, models, print_sr7, spare_parts, plate_filterBox,basic_halfCards,plate_halfCards,plate_mSR7i2n_mSR7i2u_mSR7i2d]
preview=true;  // iba na schovanie sekcie "preview"
// seriove cislo
serial_number="20260217-3";
// Nazov / Venovanie
code_name="015-mSR7-i2d";
...
halfCardWidth=40; // parametre polovicnej karty
halfCardDepth=50;
halfCardHight=4;
halfCardTick=1.0;
...
// tieto boli pouzite pri tlaceni prototypu
...
// tieto boli pouzite pre tlacenie nahradnych dielov k prototypu
//fanWidth=lineCardWidth/2-2*spacing; // ==filterWidth ==psuWidth ...pouziva sa iba fan*
//fanHight=lineCardWidth/2-2*spacing; // ==filterHight ==psuHight -//-
//fanDepth=lineCardDepth/2-2*spacing; // ==filterDepth ==psuDepth -//-

psuTrayDepth=lineCardDepth+border+fanDepth+border;
psuTrayWidth=lineCardWidth+2*border;
psuTrayHight=fanHight+2*border;
filterTick=border-2*spacing;    //hrubka filtra
filterHight=fanHight-2*border-2*spacing; //vyska filtra
filterWidth=lineCardWidth-2*border-2*spacing; //sirka filtra
...
// SR7 dimmensions
sr7Width=lineCardWidth+(2*border);
sr7Depth=lineCardDepth+border+fanDepth+border;
sr7Hight=7*lineCardHight+(2*border)+psuTrayHight;
...
module chassisCardTray(slots=7,fans=2,psu=1,label=true) {
    ...
    color(color_grey) { difference()  {
        ...
        x1=(lineCardWidth/2)+border-lineCardWidth/6;
        w1=lineCardWidth/3;
        h1=holder;
        for(z=[0:1:slots-1]) {
        ...
        translate([x3a,y3,z3]) // lavy 
            rotate([0,0,45])   // ak zle, tak rotaciu odstranit
        ...
        translate([lineCardWidth/2+border,3*border,0]) //spodny predny
            rotate([45,0,0])   // ak zle, tak rotaciu odstranit
...
//                text("sr.50",size=f1_size,font="arial");
            translate([psuTrayWidth,4*border,tz1-f1_size-f2_size-5*border])
                rotate([90,0,90])
                linear_extrude(1)
                text(serial_number,size=f2_size,font="arial");
            translate([psuTrayWidth,4*border,tz1-f1_size-2*f2_size-6*border])
                rotate([90,0,90])
                linear_extrude(1)
                text(code_name,size=f2_size,font="arial");
  ...
  union() {
    color(color_grey) { difference()  {
        ...
        for(z=[0+border:3*lineCardHight:lineCardHight*(slots-3)]){
            ...
            cube([lineCardWidth+(2*spacing),fanDepth+1,fanHight+(2*spacing)]);
        }
        ...
        // "vyhryzy" pre spojovacie hranoly suchej spojky tray-a pre linekarty
        y3=fanDepth+border;
        x3a=border*3;
        x3b=lineCardWidth-border;
        z3=((lineCardHight*slots)+(2*border))/2;
        ...
        //spodny predny vyhryz suchej spojky s power/filter-trayom
        translate([lineCardWidth/2+border,3*border,0]) 
            rotate([45,0,0])   // ak zle, tak rotaciu odstranit
            cube([lineCardWidth-2*border,holder,holder],center=true);
        //spodny zadny vyhryz suchej spojky  s power/filter-trayom
    ...
    color(color_red) {
        for(z=[0+border:3*lineCardHight:lineCardHight*(slots-3)]) {
            ...
            translate([xlk,border*5,z-spacing])
                cube([inder,border*2,inder],center=true);
            //prava dolna kolajnica
            translate([xpk,border*5,z-spacing]) 
                cube([inder,border*2,inder],center=true);
            //lava stredna kolajnica
            translate([border-spacing,border*5,z+(fanHight/2)]) 
                cube([inder,border*2,inder],center=true);
             //prava stredna
            translate([border+lineCardWidth+spacing,
                        border*5,
                        z+(fanHight/2)])
                cube([inder,border*2,inder],center=true);
            //lava horna kolajnica
            translate([xlk,border*5,z+fanHight+2*spacing]) 
                cube([inder,border*2,inder],center=true);
            //prava horna kolajnica
            translate([xpk,border*5,z+fanHight+2*spacing]) 
                cube([inder,border*2,inder],center=true);
        }
    } //color_red
...
module chassisPsuTray(slots=7,fans=2,psu=1) {
  union() {
    ...
    color(color_red) {
        ...
        translate([border-3*spacing,psuTrayDepth/2,psuTrayHight/2])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
        // pravy pridrzaci pas
        //translate([lineCardWidth+border+spacing,psuTrayDepth/2,psuTrayHight/2])
        translate([lineCardWidth+border+3*spacing,psuTrayDepth/2,psuTrayHight/2])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
        // spodny lavy pridrzaci pas
        //translate([xlk,psuTrayDepth/2,border])
        translate([xlk,psuTrayDepth/2,border-2*spacing])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
        // spodny pravy pridrzaci pas
        //translate([xpk,psuTrayDepth/2,border])
        translate([xpk,psuTrayDepth/2,border-2*spacing])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
        // horny lavy pridrzaci pas
        //translate([xlk,psuTrayDepth/2,psuTrayHight-border])
        translate([xlk,psuTrayDepth/2,psuTrayHight-border+2*spacing])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
        // horny pravy pridrzaci pas
        //translate([xpk,psuTrayDepth/2,psuTrayHight-border])
        translate([xpk,psuTrayDepth/2,psuTrayHight-border+2*spacing])
            cube([inder,psuTrayDepth-4*border,inder], center=true);
...
module filterBox(plates=3,logo=true) { color(color_filter) {
   ...
   difference() {
       translate([0,-2*border,0])  // hlavny kvader z ktoreho rezeme
           cube([lineCardWidth-2*spacing,fanDepth+2*border,fanHight-2*spacing]);
...
module filterPlate() { color(color_plate) {
    ...
    translate([border,-1,border])
        cube([filterWidth-2*border,filterTick+2,filterHight-2*border]);
    // vyrezy pre nechty
//    translate([border,-1,border/4-griddia/4]) // dolny predny
//        cube([lineCardWidth-4*border,griddia+1,griddia]);
...
//        cube([lineCardWidth-4*border,griddia+1,griddia]);
    ...
    for(x1=[border:griddia*2:lineCardWidth-3*border]) {
        translate([x1,border-griddia-2*spacing,border])
            cube([griddia,griddia,fanHight-4*border]);
    } // for x1
    for(z1=[border:griddia*2:fanHight-3*border]) {
        translate([border,border-griddia-2*spacing,z1]) 
            cube([lineCardWidth-4*border,griddia,griddia]);
    } // for z1
    } // union
...
module shieldModule() {
   ...
   translate([border*1.5,-2*spacing,lineCardTick]) // predna krytka
        ...
        cube([lineCardWidth/2-3*border-2*spacing,border-2*spacing,lineCardHight-border]);
...
// sekcia "napozovania" jednotlivych komponent , if ma iba folduje
module busHalfConnector() {
    color(color_socket) {  // lava cast
    translate([2*border,halfCardDepth-2*border-spacing,halfCardTick])
        cube([halfCardWidth*2/5,border,border]);
    } // color_socket
    color(color_socket) {  // prava cast
    translate([halfCardWidth*3/5,halfCardDepth-2*border-spacing,halfCardTick])
        cube([halfCardWidth*3/11,border,border]);
    } // color_socket
}
module shieldHalfModule() {
   color(color_module) {
   translate([border*1.5,-2*spacing,halfCardTick]) // predna krytka
        //cube([halfCardWidth/2-3*border-2*spacing,border,halfCardHight-border]);
        cube([halfCardWidth/2-3*border-2*spacing,border-2*spacing,halfCardHight-border]);
    translate([border+spacing,spacing,halfCardTick]) // spodna doska
        cube([halfCardWidth/2-2*border-2*spacing,halfCardDepth/2,halfCardTick]);

   } //color_module
}
module mdaHalfModule() {
    translate([0,0,0])
        shieldHalfModule();
    translate([2*border,halfCardDepth/4,2*halfCardTick])
        fp2Chip();
    translate([halfCardWidth/4-border,0,halfCardHight/2])  // lavy cierny port
        color(color_socket) cube([border,2.5*border,border],center=true);
    translate([halfCardWidth/4+border,0,halfCardHight/2])  // pravy cierny port
        color(color_socket) cube([border,2.5*border,border],center=true);
    translate([halfCardWidth/4-1.5*border,0.1,halfCardHight/2]) //zadne "puzdro/komora" pre porty
        color(color_socket) cube([3*border,4*border,border-0.1],center=false);
}
module isaHalfModule() {
    translate([0,0,0])
        shieldHalfModule();
    translate([2*border,border*2,2*halfCardTick])
        scale([1,2,1]) fp2Chip();
}
module shieldHalfCard() { color(color_shield) {
    union() {
        translate([0,0,0])  // spodna doska lineKarty
            cube([halfCardWidth,halfCardDepth,halfCardTick]);
        translate([0,0,0])  // predne celo lineKarty
            cube([halfCardWidth,border,halfCardHight]);
        translate([0,-2*holder,0])   // lavy pakovy uchyt karty
            cube([2*holder,2*holder,halfCardTick]);
        translate([0,-2*holder,0])
            cube([4*holder,holder,halfCardTick]);
        translate([halfCardWidth-2*holder,-2*holder,0]) //pravy pakovy uchyt karty
            cube([2*holder,2*holder,halfCardTick]);
        translate([halfCardWidth-4*holder,-2*holder,0])
            cube([4*holder,holder,halfCardTick]);
        translate([holder,0,halfCardHight/2])   // lava skrutka uchytu
            rotate([90,0,0])
            cylinder(h=holder,d=holder,center=true);
        translate([halfCardWidth-holder,0,halfCardHight/2]) // prava skrutka uchytu
            rotate([90,0,0])
            cylinder(h=holder,d=holder,center=true);
    }
} } // color_shield module
module cpmHalfCard() {
    ...
    translate([0,0,0]) // zaklad dosky
        shieldHalfCard();
    translate([2.5*holder,-1,halfCardHight/2-rj45Hight/2]) // OOB
        cube([rj45Width,rj45Depth+2,rj45Hight]);
    translate([4.0*holder,-1,halfCardHight/2-rj45Hight/2]) // Console
        cube([rj45Width,rj45Depth+2,rj45Hight]);
    translate([halfCardWidth/2-holder*4,-1,halfCardHight/2-holder/4]) // FC1 diera
        cube([holder*3,border+2,holder/2]);
    translate([halfCardWidth/2,-1,halfCardHight/2-holder/4])          // FC2 diera
        cube([holder*3,border+2,holder/2]);
//    translate([halfCardWidth/2+holder*4,-1,halfCardHight/2-holder/4]) // FC3 diera
//        cube([holder*3,border+2,holder/2]);           // je vlozena preto za koment
    } //difference
    busHalfConnector();
    xi=halfCardTick/3;
    ...
    color(color_socket) {
    translate([2.5*holder-xi,border,halfCardTick]) // OOB
        cube([holder+2*xi,border+2,halfCardHight/2]);
    translate([4.0*holder-xi,border,halfCardTick]) // Console
        cube([holder+2*xi,border+2,halfCardHight/2]);

    translate([halfCardWidth/2-holder*4-xi,border,halfCardTick]) // FC1 citacka
        cube([holder*3+2*xi,border*4,halfCardHight/2]);
    translate([halfCardWidth/2-xi,border,halfCardTick])          // FC2 citacka
        cube([holder*3+2*xi,border*4,halfCardHight/2]);
    translate([halfCardWidth/2+holder*4-xi,border,halfCardTick]) // FC3 citacka
        cube([holder*3+2*xi,border*4,halfCardHight/2]);
    translate([halfCardWidth/2+holder*4,-1,halfCardHight/2-holder/4]) // FC3 -vlozena
        cube([holder*3,border+2,holder/2]);
    } //color_socket


    color(color_chip) {
    translate([halfCardWidth/5,halfCardDepth*3/4,halfCardTick]) //zadny rad ...asi FP chipov 
        cube([3*holder,3*holder,2*xi]);
    translate([halfCardWidth/5+4*holder,halfCardDepth*3/4,halfCardTick]) // stvorcovych
        cube([3*holder,3*holder,2*xi]);                                  // obvodov
    translate([halfCardWidth/5+8*holder,halfCardDepth*3/4,halfCardTick])
        cube([3*holder,3*holder,2*xi]);
    translate([halfCardWidth/5+12*holder,halfCardDepth*3/4,halfCardTick])
        cube([3*holder,3*holder,2*xi]);

    translate([halfCardWidth/5,halfCardDepth*3/4-4*holder,halfCardTick]) //predny rad ...asi FP chipov
        cube([3*holder,3*holder,2*xi]);
    translate([halfCardWidth/5+4*holder,halfCardDepth*3/4-4*holder,halfCardTick])
        cube([3*holder,3*holder,2*xi]);
    translate([halfCardWidth/5+8*holder,halfCardDepth*3/4-4*holder,halfCardTick])
        cube([3*holder,3*holder,2*xi]);
    translate([halfCardWidth/5+12*holder,halfCardDepth*3/4-4*holder,halfCardTick])
        cube([3*holder,3*holder,2*xi]);

    
    x2=halfCardWidth/3+2*holder;
    translate([x2,halfCardDepth/2-holder,halfCardTick])  //tri asi procesory
        cube([2*holder,2*holder,3*xi]);
    translate([x2-3*holder,halfCardDepth/2-holder,halfCardTick])
        cube([2*holder,2*holder,3*xi]);
    translate([x2,halfCardDepth/2-4*holder,halfCardTick])
        cube([2*holder,2*holder,3*xi]);

    translate([x2+3*holder,halfCardDepth/2+1*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2+0*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-1*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-2*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-4*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-5*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-6*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    translate([x2+3*holder,halfCardDepth/2-7*holder,halfCardTick])  //pamatove moduly
        cube([halfCardWidth*5/16,holder/3,4*xi]);
    } //color_chip
}
module mdaHalfCard() { 
    ...
    shieldHalfCard();
    busHalfConnector();
        //translate([halfCardWidth/2-2*holder,halfCardDepth*2/3,halfCardTick+0.5])
        //    fp2Chip();
    color(color_socket) {
    translate([border*1.5,halfCardDepth/2,halfCardTick])
        cube([halfCardWidth/2-3*border,border,border]);
    translate([border*1.5+halfCardWidth/2,halfCardDepth/2,halfCardTick])
        cube([halfCardWidth/2-3*border,border,border]);
    } //color_socket
    color(color_chip) {
        for(x1=[2*border:2*border:halfCardWidth-3*border]) {
            translate([x1,halfCardDepth/2+2*border,halfCardTick])
                cube([border,halfCardDepth/2-5*border,border]);
        } //for_x1
    }
    translate([0,0,0])
        mdaHalfModule();
    translate([halfCardWidth/2,0,0])
        mdaHalfModule();
    } // union
}
...
if(setup=="design_tray_fan") {
    translate([44,52+50,0]) rotate([0,0,180]) // pri osovom krizi kartova chassis 
        chassisCardTray(7,2,1);

    translate([0,0,0])                        // pri osovom krizi ventilatorova chassis
        chassisFanTray(7,2,1);

    translate([-20,-20,0]) rotate([0,0,180]) // ventilatorova chassis vlavo od kriza
        chassisFanTray(7,2,1);

    translate([-20-psuTrayWidth-20,-20+lineCardDepth-lineCardDepth,0]) rotate([0,0,180]) 
        chassisCardTray(7,2,1);    // kartova chassis vlavo od kriza

    translate([200,0,0]) 
        chassisCardTray(slots=7,fans=2,psu=1);
    translate([200+lineCardWidth+2*border,lineCardDepth+border+fanDepth+border,0])
        rotate([0,0,180]) 
...
if(setup=="design_filters") {
    ...
    //leziaci plat filtra
    translate([border,-1.5*fanHight,0])
        rotate([-90,0,0])
        filterPlate();
...
if(setup=="print_sr7") {
    ...
    // spojovacie pasiky 
    //fix0        translate([0,7*lineCardHight+2*border+70+lineCardDepth,0])
    //fix0            chassisAdhesiveBars(slots=7,fans=2,psu=1);
    //fix0        translate([1.7*lineCardWidth,7*lineCardHight+2*border+6,0])
    //fix0            chassisAdhesiveBars(slots=7,fans=2,psu=1);
    //fix2 translate([4*psuTrayWidth+6*border,2*lineCardDepth+7*border,0])
    //fix2     chassisAdhesiveBars(slots=7,fans=2,psu=1);
    //fix0        translate([5*psuTrayWidth+7*border,2*lineCardDepth+7*border,0])
    //fix0            chassisAdhesiveBars(slots=7,fans=2,psu=1);
    //fix2 translate([4*psuTrayWidth+7*border,3*lineCardDepth+5*border,0])
    //fix2     chassisAdhesiveBars(slots=7,fans=2,psu=1);
    //fix2 translate([3*psuTrayWidth+7*border,3*lineCardDepth+5*border,0])
    //fix2     chassisAdhesiveBars(slots=7,fans=2,psu=1);
...
if(setup=="spare_parts") { // bezne nahradne diely
    // boxy FAN a PSU
    translate([border+0*(2*border+fanWidth),2*border,0])
        fanBox();
    translate([border+1*(2*border+fanWidth),2*border,0])
        fanBox();
    translate([border+2*(2*border+fanWidth),2*border,0])
        psuBox();
    translate([border+3*(2*border+fanWidth),2*border,0])
        psuBox();
    // karty
    translate([0*(lineCardWidth+border),1*(fanDepth+6*border),0])
        mdaCard();
    translate([1*(lineCardWidth+border),1*(fanDepth+6*border),0])
        shieldCard();
    // nahradne filtre
    translate([border+4*(2*border+fanWidth),2*border+0*filterHight,filterTick])
        rotate([-90,0,0])
        filterPlate();
    translate([border+4*(2*border+fanWidth),3*border+1*filterHight,filterTick])
        rotate([-90,0,0])
        filterPlate();
    translate([border+4*(2*border+fanWidth),4*border+2*filterHight,filterTick])
        rotate([-90,0,0])
        filterPlate();
}
if(setup=="plate_filterBox") {
    // len kvoli urvanym uskam
    translate([0,0,0])
        filterBox();
}
if(setup=="basic_halfCards") {
    // zakladne zobrazenie chassis s polovicnymi kartami
    translate([0,0,0])
        chassisCardTray();
    xx1=border;
    zz1=border;
    translate([xx1,0,zz1])
        shieldHalfCard();
    zz2=zz1+halfCardHight;
    translate([xx1,0,zz2])
        cpmHalfCard();
}
if(setup=="plate_halfCards") {
    xx0=0;
    yy0=0;
    yy1=halfCardDepth+10;
    translate([xx0,yy0,0])
        mdaHalfCard();
    translate([xx0,yy1,0])
        cpmHalfCard();

    xx1=halfCardWidth+10;
    translate([xx1,yy0,0])
        isaHalfCard();
    translate([xx1,yy1,0])
        shieldHalfCard();
...
if(setup=="plate_mSR7i2n_mSR7i2u_mSR7i2d") {
    sr7dx=sr7Width+30;
    //w0=sr7dx*0;  translate([w0,       0,0])  rotate([  0,0,0])  chassis(7,2,1);

    w1=sr7dx*1;  translate([w1,sr7Hight,0])  rotate([ 90,0,0])  chassis(7,2,1);

    //w2=sr7dx*2;  translate([w2,0,sr7Depth])  rotate([-90,0,0])  chassis(7,2,1);
}
/* [Zoznam chyb] */
// 1. modul filtrov nema drazky v strede ... oprave
// 2. vo vyklenkoch/slotoch pre ventilatory je vodiaca lista posunuta nad stred .... malo by byt opravene
// 3. vodiace listy su zbytocne velke .... opravene
// 4. spacing zdrojov je v podstate dobry. Mozno malinko uvolnit (zvacsit stacing)
// 5. spacing ventilatorov je nedostatocny. pozrietOB, ci je to chyba rozmerov ventilatora, alebo vyklenku ..... malo by byt opravene 289:
// 6. rozmery filtra ... uz opravene
// 7. vodiace vlastne vsetky listy vycnievaju moc. Drazka boxov zacina hlbsie .... malo by byt opravene (posunute o 2xspac hlbsie
// 8. vnutorna sucha spojka Tray-ov ... je kompletne nevyhovujuca .... pre zatial vsetky vyhryzy otocene 45', rozmery na holder
// 9. realisticka vyska linekarty by bola vhodna 4mm miesto 8 .... az v tretej iteracii
//10. hlbka zdrojov a ventilatorov by bola vhodna 10mm miesto 25 ... az v tretej iteracii
//11. priechody medzi ventilatormi a linekartami su nakrivo ... opravene
//12. lopatky ventilatora niesu polozene na jednej Y-revine, ale vystupuju vpred
//13. vo vnutri ventilatora je nejaky divny support
//14. farby nieje potrebne zohladnovat pre tuto chvilu .... filament sa aj tak riesi osobitne
//15. dorobit SLICER s reznymi rovinami pre kontrolu vnutornych casti
//16. powerTray vodiace listy posunute hlbsie ...fix2
//17. znizit vysku LineKariet o spacing
// --- end ---
