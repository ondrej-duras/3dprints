...
/* [Preview] */
preview="basic"; // [basic2,basic4,hinges,detail,movie,plate2,plate4,plateRoof]
doorPull=10;     // [0:40]
roofPull=0;      // [0:40]
...
cabTick=1.4;   // [1:0.1:3] 2.0 hrubka steny kabinetu 
...
pantStick=2.7; // [0.2:0.1:5.0] hrubka pantoveho valca
...
spi=spacingI;
pad=padding;
supportTick=0.4; // hrana oporneho hranolceka (supportu)
...
module leftPant(support=0) { // pant na dverach otvaranych do lava
   difference() { 
        union() {
            ...
            if(support!=0) {
                translate([pantDepth/2,-1.5,pantHight-supportTick])  // horny support pantu
                    cube([supportTick,cabTick+1.5,supportTick]);
                translate([pantDepth/2,-1.5,0])                      // dolny support pantu
                    cube([supportTick,cabTick+1.5,supportTick]);
            }
        }
...
module rightPant(support=0) { // pant na dverach otvaranych do prava
   // referencny bod pantu je na dolnej hrane medzi plochy kovanim a valcovou objimkou v vrchnej(viditelnej strany)
   difference() { 
        union() {
            ...
            translate([pantDepth/2,-pantDepth/2-hingeSpi-hingeSpo,0])  // a cylinder vonkajsej casti
                cylinder(h=pantHight,d=pantDepth-hingeSpi-hingeSpo);
            if(support!=0) {
                translate([pantDepth/2-supportTick,-1.5,pantHight-supportTick])  // horny support pantu
                    cube([supportTick,cabTick+1.5,supportTick]);
                translate([pantDepth/2-supportTick,-1.5,0])  // spodny support pantu
                    cube([supportTick,cabTick+1.5,supportTick]);
            }
        }
        translate([pantDepth/2,-pantDepth/2-hingeSpi-hingeSpo,-1]) // diera
...
module cabinetRoof() {  // v polohe na vyrobnej doske, alebo ako spodny diel v zakladnej polohe
    // referencnym bodom [0,0] je lavevy predny spodny roh suciastky
    union() {
        translate([0,0,0])
        ...
        difference() {
            ...
            translate([cabTick*2-spo,cabTick*2-spo,cabTick])
                cube([cabWidth-4*cabTick+2*spo,cabDepth-4*cabTick+2*spo,doorBase+1]);
        }
    }
}

module cabinetRoofTop() { // otoceny ako vrchny diel
    // referencnym bodom [0,0] je lavy predny spodny roh na sirokej doske, nie obrube
...
module doorLeft(support=0) {
    color(colorDoor)
    union() {
        translate([0,0,0])   // doska dveri
            cube([doorWidth,doorTick,doorHight]);    
        translate([-pantDepth,0,doorHight*6/7+spi])  // horny pant
            leftPant(support);
        translate([-pantDepth,0,doorHight*1/7+spi])  // dolny pant
            leftPant(support);
...
if(preview=="plateRoof") {
    color("#800000") difference() {
    translate([0,0,0])
        cabinetRoof();
    translate([cabWidth/2,-1,-1])
        cube([cabWidth,cabDepth+2,cabHight+2]);
    }
}
