...
spp=5.0;            // rozostup medzi vecami na vyrobnej doske
preview="basic";    // [basic,cover,slider,parking,plate,crowBar]
// vzdialenost kritky od podlozky
distance=10;        // [0:1:50] 
numY=2; // [0:1:10]
numX=2; // [0:1:10] 
/* [Dimensions] */
roundedRadius=1.5;  // polomer zabrusenia hran slidera
roundedRadiusBig=3.5; // =< (coverTick+ferulaHight)/2 
...
ferulaHight=5.0;    // REFERENCNY rozmer - vyska vnutorneho pristoru pod coverom/krytkou ... urcuje vysku krytku
coverTick=2.0;      // hrubka materialu cover-a  skrutka 7.1=2(hlava)+5.1(zavit)

parkWidth=50;       // sirka parkovacej dosky
parkDepth=60;       // dlzka (hlbka do Y) parkovacej dosky
parkHight=2.0;      // hrubka parkovacej dosky (normalne by to mozno mal byt parkTick)
parkTick=2.0;       // materialova hrubka parkovacej dosky - hrubka povrchovych casti, prikryvajucich slider z vrchu
/* [Dimensions_crowBar] */
// dlzka paky pacidla
cbArm=80.0; // [0:10:250]
// dlzka hrotu pacidla
cbTip=5.0;  // [0:1:100]
// polomer zaoblenia pacidla
cbRad=5.0;  // [1:1:30]
// sirka pacidla
cbExt=5.0;  // [1:1:100]
// pocet pacidiel na doske
cbNumX=3;
...
colorParkingOut="#808020";   // farba parkovacej dosky
colorParkingInn="#505010";   // farba parkovacej dorky - vnutorne plochy
...
module parking() {  // parkovisko pre slider - zachytna podlozka, ktoru mozno prilepit 
    color(colorParkingOut)
    cube([parkWidth,parkDepth,parkHight]); // zakladna podlozka parkovacej dosky
    
    color(colorParkingInn)
    translate([parkWidth/2-sliderWidthM/2-parkTick,0,parkHight]) // teleso, do ktoreho bude vyrezany priestor pre slider
        cube([sliderWidthM+2*parkTick,sliderDepthL+sliderDepthR,sliderHightB+parkTick]);
}

/* [Preview] */
if(preview=="basic") {
    slider();     
    translate([sliderWidthT+(sliderWidthM-sliderWidthT)/2,0,ferulaHight+coverTick+sliderHightB+distance+coverTick]) rotate([0,180,0]) cover();
    //sliderWidthB,sliderDepthM,sliderHightB
}
if(preview=="slider") {
    slider();
}
...
if(preview=="plate") {
    dy=sliderDepthL+distance+sliderDepthR+spp;
    dx=sliderWidthM+distance+sliderWidthT+spp;
    
    for(y=[0:dx:numY*dx-0.001]){
        for(x=[0:dx:numX*dx-0.001]){
            translate([x,y,0])
                slider();
            
            translate([x+sliderWidthM+distance,y,0])
                cover();
        } //x
    } //y
}
