...
boxWidth=30.0;  // [30:10:150]
// hlbka/dlzka krabicky - vonkajsi rozmer boxPool spodnej casti
boxDepth=50.0;  // [30:10:200] 
// vyska krabicky - vonkajsi rozmer boxPool spodnej casti
boxHight=70.0;   // [10:1:50]
// polomer zaoblenia vsetkych krabicky
boxRound=7.0;    // [0:0.1:20]
// hrubka materialu krabicky ...boxCover si ju musi pripocitat
boxTick=1.5;     // [0.5:0.1:5.0]
// hlbka vyrezu deklika - vzdialenost valca
boxCutOutHC=0.80;  // [0:0.05:1.5]
// hlbka vyrezu deklika - polomer valca
boxCutOutDC=0.45;   // [0:0.05:1.5]
// hlbka vyrezu vanicky - vzdialenost valca
boxCutOutHP=0.65;  // [0:0.05:1.5]
// hlbka vyrezu vanicky - polomer valca
boxCutOutDP=0.20;  // [0:0.05:1.5]

boxName="Danka";  //Napis na dekliku aj vanicke
...
module boxPool(bw=boxWidth,bd=boxDepth,bh=boxHight,bt=boxTick,br=boxRound,ci=colorBoxPoolInt,co=colorBoxPoolOut,tt=boxName,tf=boxFont) {
    ...
    difference() {
        ...
        color(ci)
            ...
            rotate([-90,0,0])
            cylinder(h=bd+2,d=bw*boxCutOutDP);
        color(ci)
            translate([bt/4,br,br])
            rotate([0,-90,0])
            linear_extrude(1)
            text(tt,size=tf,font="arial");
    } // diff
}
