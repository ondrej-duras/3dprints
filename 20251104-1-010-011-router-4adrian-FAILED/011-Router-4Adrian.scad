...
module cpmCard() {
    ...
    translate([4.0*holder,-1,lineCardHight/2-holder*3/4]) // Console
        cube([holder,border+2,holder]);
    translate([lineCardWidth/2-holder*4,-1,lineCardHight/2-holder/4]) // FC1
        cube([holder*3,border+2,holder/2]);
    translate([lineCardWidth/2,-1,lineCardHight/2-holder/4])          // FC2
        cube([holder*3,border+2,holder/2]);
    translate([lineCardWidth/2+holder*4,-1,lineCardHight/2-holder/4]) // FC3
        cube([holder*3,border+2,holder/2]);
    } //difference
    busConnector();
    xi=lineCardTick/3;
    ...
    translate([lineCardWidth/2-holder*4-xi,border,lineCardTick]) // FC1
        cube([holder*3+2*xi,border*4,lineCardHight/2]);
    translate([lineCardWidth/2-xi,border,lineCardTick])          // FC2
        cube([holder*3+2*xi,border*4,lineCardHight/2]);
    translate([lineCardWidth/2+holder*4-xi,border,lineCardTick]) // FC3
        cube([holder*3+2*xi,border*4,lineCardHight/2]);
    } //color_socket
    ...
    translate([lineCardWidth/5+4*holder,lineCardDepth*3/4,lineCardTick])
        cube([3*holder,3*holder,2*xi]);
...
module mdaCard() { 
    ...
    busConnector();
    color(color_socket) {
    translate([border,lineCardDepth-2*border-spacing,lineCardTick])
        cube([lineCardWidth-2*border,border,border]);
    } // color_socket
    color(color_chip) {
        translate([lineCardWidth/2-2*holder,lineCardDepth*2/3,lineCardTick+0.5])
            cube([4*holder,4*holder,lineCardTick]);
    }
    } // union
}
...
if(preview==true) { // sekcia "napozovania" jednotlivych komponent , if ma iba folduje
    ...
    if(setup=="print_sr7") {
        ...
        translate([0*psuTrayWidth,th1+20+fanDepth,0])
            filterBox();
        translate([1*psuTrayWidth,th1+25+fanDepth,border])
            rotate([-90,0,0])
            filterPlate();
        translate([2*psuTrayWidth,th1+25+fanDepth,border])
            rotate([-90,0,0])
            filterPlate();
        translate([3*psuTrayWidth,th1+25+fanDepth,border])
            rotate([-90,0,0])
            filterPlate();
